    ...
    respond_to do |format|
      if @matches_creator.save
        ...
        @user_inputs.each do |input|
          fields = input.split(" ")
          first_team = fields[0]
          first_team_id = Team.find_by(name: first_team) == nil ? -1 : Team.find_by(name: first_team).id.to_i
          second_team = fields[1]
          # second_team_id = Team.find_by(name: second_team).id.to_i
          second_team_id = Team.find_by(name: second_team) == nil ? -1 : Team.find_by(name: second_team).id.to_i
